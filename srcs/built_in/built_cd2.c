...
/*   Updated: 2022/10/23 16:34:50 by seungsle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
					...
					t_env *env_pwd)
{
	...
	if (check_cd_error(data, cmd, result))
		return (1);
	cd_check_else_sub(data->pwd, env_oldpwd, env_pwd);
	return (0);
}
					...
					t_env *env_pwd)
{
	...
	change_env(env_oldpwd, pwd);
	newpwd = getcwd(0, 1024);
	change_env(env_pwd, newpwd);
	free(newpwd);
}
