...
/*   Updated: 2022/10/23 16:25:15 by seungsle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			cd_check_dash(t_data *data, char **cmd, t_env *env_oldpwd, \
					t_env *env_pwd);
...
void		built_export_sub(char *ptr, t_data *data, char **cmd2);
...
int			append_token(t_token_list *list, t_parse *parse, \
...
*** parse ***
*/
void		replace_util_sub1(t_parse *parse, char *first, \
		char *val, char *last);
void		replace_util_sub2(t_parse *parse, char *buf_start, \
		char *buf_env, char *buf_end);
void		replace_dollar_options(t_data *data, t_parse *parse, \
		char *buf_start, char *buf_end);
...
char		*ft_substr(char const*s, unsigned int start, size_t len);
int			parse_dollar2(char *temp, char **ret, \
		struct s_intset_jh save, t_data *data);
void		parse_dollar(char *temp, char **ret, \
		struct s_intset_jh save, t_data *data);
int			parse_dollar2_b(char *temp, char **ret, \
		int count, int save_c);
void		parse_backslash(char *temp, char **ret, \
		struct s_intset_jh save, int *bs);
void		cmd_heredoc_write(struct s_dollar_str t, \
		char *ret, char *s, t_data *data);
...
t_token		***make_t_cmd_make_lst(t_token_list *tok_lst, \
		t_cmd_make **start, int a, int b);
void		whatisit(t_token_list *t, t_token ***cmd_l, \
...
void		exe_cmd3(char *temp2, char **temp, \
		char **cmd, char **path);
int			monitoring_pid(pid_t last_pid, int ret);
pid_t		exe_fork2(t_token ***cmd_lst, t_data *data, \
		int count, int *fd);
